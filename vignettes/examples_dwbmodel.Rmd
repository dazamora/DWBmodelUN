...
setup_data <- readSetup(Read = TRUE)
Dates <- seq(as.Date(colnames(P_sogamoso)[3], format = "%Y.%m.%d"), 
             as.Date(tail(colnames(P_sogamoso),1), format = "%Y.%m.%d"), 
             by = "month")
...
result <- dds(xBounds.df = xBounds.df, numIter=20, OBJFUN=NSE_Sogamoso_DWB,
