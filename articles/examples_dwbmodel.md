...
`DWBCalculator` is the function responsible for incorporating the DWB
model calculations into the **DWBmodelUN** package. In this connection,
the following example corresponds to an ungauged modeling exercise
developed for the Sogamoso River basin (SRB) by (Duque, 2018).

As a first step, the example starts with loading datasets generated for
SRB that are required by DWB model and subsequently, the coordinates
system of the input data are verified. The last step is to run
`DWBCalculator` function in order to obtain a monthly streamflow
simulated.
...
#> Warning in Coord_comparison(P_sogamoso, PET_sogamoso): First date/header does
#> not match - Please verify
#> Warning in Coord_comparison(P_sogamoso, PET_sogamoso): Final date/header does
#> not match - Please verify
#> Extent verified
#> Resolution verified
...
`dds` is the function that implements the calibration algorithm
Dynamical Dimension Search by (Tolson and Shoemaker, 2007). In the
example, the DWB model parameters are calibrated in the SRB with 10
Group Response Unit (GRU). The Nash-Sutcliffe efficiency (`nse.cof`) is
created as an objective function between simulated and observed
streamflow data, with the treatment of missing values and its coupled
with the `DWBCalculator` function. Finally, the calibration is done with
2 iterations using the databases of the sample basin.
...
#> Warning in cellBasins(GRU, basins): gruLoc and basins have different coordinate
#> reference systems. The basins are reprojected to the raster CRS during the
#> extraction, but verify projections before using the results.
...
`graphDWB` is the function in charge of creating the graphical inputs
and outputs of **DWBmodelUN**. The function has four types of graphs to
be generated:

1.  The first one (`tp = 1`) corresponds to a hydrograph of any
    continuous variable, such as the precipitation entries.
2.  The second one (`tp = 2`) compares the simulated runoff with the
    observations.
3.  The third one (`tp = 3`) generates a combined chart with a rain
    hyetograph and the simulated and observed streamflow hydrographs.
4.  The fourth one (`tp = 4`) adds a hydrograph of evapotranspiration to
    the previous comparison.

The present example shows how to generate three of the four chart types,
using the datasets included in **DWBmodelUN** for the SRB. To draw one
of the chart types it is necessary to load the variables involved in
each one, to adjust the input series to a time vector and assign them to
a list element. Finally to run `graphDWB`.
...
library(dygraphs)
data(P_sogamoso)
P.est <- ts(c(t(P_sogamoso[1, -2:-1])), start = c(2012, 1), frequency = 12)
...
P.est <- ts(c(t(P_sogamoso[1, -2:-1])), start = c(2012, 1), frequency = 12)
runoff.sim <- ts(simDWB.sogamoso[c(131:192) ,1], start = c(2012, 1), frequency = 12)
runoff.obs <- ts(EscSogObs[c(131:192) ,1] , start = c(2012, 1), frequency = 12)
...
P <- ts(c(t(P_sogamoso[1, -2:-1])), start = c(2012, 1), frequency = 12)
PET <- ts(c(t(PET_sogamoso[1, -2:-1])), start = c(2012, 1), frequency = 12)
runoff.sim <- ts(simDWB.sogamoso[c(131:192), 1], start = c(2012, 1), frequency = 12)
