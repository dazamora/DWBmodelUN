...
#'                     
DWBCalculator_daily_agg <- function(p_v, pet_v, g_v, s_v, alpha1_v, alpha2_v, smax_v, d_v, x4_v, calibration = FALSE, cell_Basins){
  ...
  q_routing <- routing(ndays, qd, qb, q_total, x4_v, cell_Basins)

  #---- return ----
  dwb_aux <- list(q_total = q_routing$qt, aet = aet, r = r, qd = q_routing$qd, qb = q_routing$qb, s = s, g = g)
  return(dwb_aux)
}
