...
#' 
#' 
buildGRUmaps_wapaba <- function(gruLoc, parsValues){
  ...
  alpha1 <- alpha2 <- smax <- k <- beta <-  raster::raster(gruLoc)
  
  for (i in 1:dim(parsValues)[1]){
    alpha1[gruLoc == i] <- parsValues[i, 1]
    alpha2[gruLoc == i] <- parsValues[i, 2]
    smax[gruLoc == i] <- parsValues[i, 3]
    k[gruLoc == i] <- parsValues[i, 4]
    beta[gruLoc == i] <- parsValues[i, 5]
  }
  ...
  k_v <- raster::rasterToPoints(k)[,-c(1,2)]
  beta_v <- raster::rasterToPoints(beta)[,-c(1,2)]
  
  gruMaps <- list(alpha1 = alpha1_v, alpha2 = alpha2_v, smax = smax_v, k = k_v, beta = beta_v,
                  alpha1R = alpha1, alpha2R = alpha2, smaxR = smax, kR = k, betaR = beta)
  return(gruMaps)
}
