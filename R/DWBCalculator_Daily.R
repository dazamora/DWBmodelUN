...
#' 
#' @title
#' DWB model function - Daily version
#'
...
#'
#'  #falta agregar parametros adicionales
#'
#'
#'
...
#'                     
DWBCalculator_daily <- function(p_v, pet_v, g_v, s_v, alpha1_v, alpha2_v, smax_v, d_v, x4_v, calibration = FALSE){
  ...
  if (!calibration){
    ...
    M.result <- matrix(nrow = ncells, ncol = ndays) 
    ...
    orduh2 <- do.call(cbind,UH(branch = 2, NH = max(x4_v)+5, C=x4_v))
    steps <- max(1, 2*(max(x4_v)+5)-1)
    h2 <- matrix(0, nrow = steps+1, ncol = ncells)
    for(k in 1:steps){
      h2[k,] <- h2[k+1,] + orduh2[k,]*qd[, 1]
    }
    h2[(steps+1),] <- orduh2[steps+1,]*qd[, 1]
    
    qd[, 1] <- unlist(lapply(h2[1,], function(x){max(0,x)}))
    ...
    orduh1 <- do.call(cbind,UH(branch = 1, NH = max(x4_v)+5, C=x4_v))
    steps <- max(1, (max(x4_v)+5)-1)
    h1 <- matrix(0, nrow = steps+1, ncol = ncells)
    for(k in 1:steps){
      h1[k,] <- h1[k+1,] + orduh1[k,]* r[, 1]
    }
    h1[(steps+1),] <- orduh1[(steps+1),]*r[, 1]

    r[, 1] <- unlist(lapply(h1[1,], function(x){max(0,x)}))
    ...
    for(i in 2:ndays){
      ...
      steps <- max(1, 2*(max(x4_v)+5)-1)
      for(k in 1:steps){
        h2[k,] <- h2[k+1,] + orduh2[k,]*qd[, i]
      }
      h2[(steps+1),] <- orduh2[(steps+1),]*qd[, i]
      
      qd[, i] <- unlist(lapply(h2[1,], function(x){max(0,x)}))
      ...
      steps <- max(1, (max(x4_v)+5)-1)
      for(k in 1:steps){
        h1[k,] <- h1[k+1,] + orduh1[k,]* r[, i]
      }
      h1[(steps+1),] <- orduh1[(steps+1),]*r[, i]
      
      r[, i] <- unlist(lapply(h1[1,], function(x){max(0,x)}))
