...
#' 
#' 
buildGRUmaps_daily <- function(gruLoc, parsValues){
  ...
  alpha1 <- alpha2 <- smax <- d <- x4 <-  raster::raster(gruLoc)
  ...
  x4_v <- raster::rasterToPoints(x4)[,-c(1,2)]
  
  gruMaps <- list(alpha1 = alpha1_v, alpha2 = alpha2_v, d = d_v, smax = smax_v, x4 = x4_v,
                  alpha1R = alpha1, alpha2R = alpha2, dR = d, smaxR = smax, x4R = x4_v)
  return(gruMaps)
}
