...
KGE_OF_wapaba <- function(parameters, P, PET, g_v,s_v, Sim.Period, EscObs, Cal.Period,Heat.Period=NA, GRU){
  ...
  if (sum(!is.na(sim)) == prod(dim(sim))){
    ...
    length(sim) == length(obs)
      ...
      erre <- cor(sim, obs, method="pearson")
      
      kge.cof <- 1 - sqrt((erre - 1)^2 + (alfa - 1)^2 + (beta - 1)^2)
