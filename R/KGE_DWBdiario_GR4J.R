...
KGE_DWBdiario_GR4J <- function(parameters, P, PET, g_v,s_v, Sim.Period, EscObs, Cal.Period,Heat.Period=NA, GRU, cell_Basins){
  ...
  GRU.maps <- buildGRUmaps_daily(GRU, param)
  ...
  if(is.na(Heat.Period[1])){
    DWB.basin <- DWBCalculator_daily_agg(p_v=P[ ,Cal.Period], pet_v=PET[ ,Cal.Period],
                                  g_v,s_v, alpha1_v, alpha2_v, smax_v,d_v, x4_v,calibration = T, cell_Basins)
  }else{
    DWB.basin <- DWBCalculator_daily_agg(p_v=P[ ,c(Heat.Period,Cal.Period)], pet_v=PET[ ,c(Heat.Period,Cal.Period)],
                                 g_v,s_v, alpha1_v, alpha2_v, smax_v,d_v, x4_v,calibration = T, cell_Basins)
  }
  
  # Agrupar la escorrentía por cada estación
  Esc.basin <- DWB.basin$q_total
  
  # Remover fechas del periodo de calentamiento 
  if(is.na(Heat.Period[1])){
    sim <- Esc.basin$varAverage
    obs  <- as.data.frame(EscObs[Cal.Period-2,])
  } else{
    a <- length(Heat.Period)
    b <- 1:a
    sim <- Esc.basin$varAverage[-b,]
    obs  <- as.data.frame(EscObs[Cal.Period-(length(Heat.Period)+2),])
  }
