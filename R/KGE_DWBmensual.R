...
KGE_DWBmensual <- function(parameters, P, PET, g_v,s_v, Sim.Period, EscObs, Cal.Period,Heat.Period=NA, GRU){
  ...
  if(is.na(Heat.Period[1])){
    DWB.sogamoso <- DWBCalculator(p_v=P[ ,Cal.Period], pet_v=PET[ ,Cal.Period],
  ...
  Esc.Sogamoso <- varBasins(DWB.sogamoso$q_total, cell_Basins$cellBasins)
  
  # Remover fechas del periodo de calentamiento 
  if(is.na(Heat.Period[1])){
    sim <- Esc.Sogamoso$varAverage
